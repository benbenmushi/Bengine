...
public abstract class Singleton<T> : MonoBehaviourEx where T : Singleton<T>, new()
{
	...
	/// <summary>
	/// Setting : If can be destroyed, then destroy only the component and keep the GameObject.
	/// </summary>
	protected static bool                   DestroyOnlyComponent;
	...
	///// </summary>
	//protected static event Action<Type>     SingletonInitializedEvent;
	...
	static Singleton()
	{
		...
		ParentName = null;
		HideInstanciated = false;
		DontDestroyOnLoadScene = false;
		DestroyOnlyComponent = false;
	...
	private static void InitializeInstance(T o)
	{
		if (!IsInitializing)
		{
			...
			m_instance.OnSingletonInitialized();
			//if (SingletonInitializedEvent != null)
			//	SingletonInitializedEvent(typeof(T));
