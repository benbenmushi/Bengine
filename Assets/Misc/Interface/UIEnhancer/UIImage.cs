...
namespace UnityEngine.UI
{
	...
	public class UIImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
	{
		...
		[SerializeField]
		private float m_slicedSize = 1.0f;
		public float slicedSize { get { return m_slicedSize; } set { if (SetPropertyUtility.SetStruct(ref m_slicedSize, value)) SetVerticesDirty(); } }
		...
		Vector4 GetAdjustedSlicedBorders(Vector4 border, Rect rect, Vector2 spriteSize)
		{
			...
			if (borderScaleRatio[0] < borderScaleRatio[1])
				return border * borderScaleRatio[0] * (combinedBorders[0] / spriteSize[0]) * slicedSize;
			else
				return border * borderScaleRatio[1] * (combinedBorders[1] / spriteSize[1]) * slicedSize;
		}
