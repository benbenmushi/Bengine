...
public static class trackedEventUtility
{
    ...
    {
        get { return UnityEditor.EditorPrefs.GetBool("ReplicaConversationConfigEditorsceneGraphFoldout", false); }
        set { UnityEditor.EditorPrefs.SetBool("ReplicaConversationConfigEditorsceneGraphFoldout", value); }
    }
    public static bool logRegistrationErrors
    {
        get { return UnityEditor.EditorPrefs.GetBool("ReplicaConversationConfigEditorCharactersFoldout", false); }
        set { UnityEditor.EditorPrefs.SetBool("ReplicaConversationConfigEditorCharactersFoldout", value); }
    }
    public const string trackedLogPrefix = "<color=#006400>\t/!\\ EVENT CALL /!\\</color>: ";
#else
	public const string trackedLogPrefix = "[TrackerEvent]: ";
	public static bool  m_logInvokeEventCalls = false;
	public static bool logInvokeEventCalls { get { return m_logInvokeEventCalls; } set { m_logInvokeEventCalls = value; } }
	static bool  m_logRegistrationErrors = false;
	public static bool logRegistrationErrors { get { return m_logRegistrationErrors; } set { m_logRegistrationErrors = value; } }
#endif
...
public class trackedEvent
{
    ...
    private event Action    _action_once;
    ...
    {
        if (_action != null || _action_once != null)
        {
            if (trackedEventUtility.logInvokeEventCalls || showDebug)
            {
                List<Delegate> dList = new List<Delegate>();
                if (_action != null)
                    dList.AddRange(_action.GetInvocationList());
                if (_action_once != null)
                    dList.AddRange(_action_once.GetInvocationList());
                Debug.Log(trackedEventUtility.trackedLogPrefix + sourceName + "." + eventName + ".InvokeEvent(showDebug=true) - " + dList.Count + " subscribed(" + dList.Select(d => d.Method.DeclaringType.Name).Aggregate((s1, s2) => s1 + ", " + s2) + ")");
            }
            if (_action != null)
                _action();
            Action once = _action_once;
            _action_once = null;
            if (once != null)
                once();
        }
    ...
    public void RegisterOnce(Action a)
    {
        if (a != null)
            _action_once += a;
    }
}
public class trackedEvent<T1>
{
...
public class trackedEvent<T1, T2>
{
    ...
    public static trackedEvent<T1, T2> operator -(trackedEvent<T1, T2> s, Action<T1, T2> a)
    {
        s.UnRegisterAction(a);
        return s;
    }
    protected void RegisterAction(Action<T1, T2> a)
    {
        if (!trackedEventUtility.IsRegistered(_action, a))
        {
            _action += a;
#if UNITY_EDITOR
            HooksTracker.StopTracking(sourceName, eventName, a.Method);
#endif
        }
        else if (trackedEventUtility.logRegistrationErrors)
            Debug.LogError(a.Method.Name + " is already subscribed from " + sourceName + "." + eventName);
    }
    protected void UnRegisterAction(Action<T1, T2> a)
    {
        if (trackedEventUtility.IsRegistered(_action, a))
        {
            _action -= a;
            ...
            Debug.LogError(a.Method.Name + " is already unsubscribed from " + sourceName + "." + eventName);
    }
}
public class trackedEvent<T1, T2, T3>
{
...
public class trackedEventAsync
{
    ...
    public Coroutine InvokeEvent(MonoBehaviour caller, Action endCallback = null, bool showDebug = false)
    {
#if UNITY_EDITOR
        return caller.StartCoroutine(InvokeEventTracked_internal(caller, endCallback, showDebug));
#else
		return caller.StartCoroutine(InvokeEvent_internal(caller, endCallback, showDebug));
#endif
    }
    ...
    private IEnumerator InvokeEventTracked_internal(MonoBehaviour caller, Action endCallback = null, bool showDebug = false)
    {
        if (caller != null)
        {
            if (_asyncAction != null && (trackedEventUtility.logInvokeEventCalls || showDebug))
            ...
            for (int i = 0; _asyncAction != null && i < _asyncAction.GetInvocationList().Length; i++)
            {
                ...
                yield return caller.StartCoroutine(coroutineEvent.Method.Name, coroutineEvent());
            }
            if (endCallback != null)
                endCallback();
        }
...
public class trackedEventAsync<T1>
{
    ...
    public Coroutine InvokeEvent(MonoBehaviour caller, T1 arg, Action endCallback = null, bool showDebug = false)
    {
#if UNITY_EDITOR
        return caller.StartCoroutine(InvokeEventTracked_internal(caller, arg, endCallback, showDebug));
#else
		return caller.StartCoroutine(InvokeEvent_internal(caller, arg, endCallback, showDebug));
#endif
    }
    ...
    private IEnumerator InvokeEventTracked_internal(MonoBehaviour caller, T1 arg, Action endCallback = null, bool showDebug = false)
    {
        if (caller != null)
        {
            if (_asyncAction != null && (trackedEventUtility.logInvokeEventCalls || showDebug))
            ...
            for (int i = 0; _asyncAction != null && i < _asyncAction.GetInvocationList().Length; i++)
            {
                ...
                yield return caller.StartCoroutine(coroutineEvent.Method.Name, coroutineEvent(arg));
            }
            if (endCallback != null)
                endCallback();
        }
...
public class trackedEventAsync<T1, T2>
{
    ...
    public Coroutine InvokeEvent(MonoBehaviour caller, T1 arg, T2 arg2, Action endCallback = null, bool showDebug = false)
    {
#if UNITY_EDITOR
        return caller.StartCoroutine(InvokeEventTracked_internal(caller, arg, arg2, endCallback, showDebug));
#else
		return caller.StartCoroutine(InvokeEvent_internal(caller, arg, arg2, endCallback, showDebug));
#endif
    }
    ...
    private IEnumerator InvokeEventTracked_internal(MonoBehaviour caller, T1 arg, T2 arg2, Action endCallback = null, bool showDebug = false)
    {
        if (caller != null)
        {
            if (_asyncAction != null && (trackedEventUtility.logInvokeEventCalls || showDebug))
            ...
            for (int i = 0; _asyncAction != null && i < _asyncAction.GetInvocationList().Length; i++)
            {
                ...
                yield return caller.StartCoroutine(coroutineEvent.Method.Name, coroutineEvent(arg, arg2));
            }
            if (endCallback != null)
                endCallback();
        }
...
public class trackedEventSynchro
{
    ...
    public Coroutine InvokeEvent(MonoBehaviour caller, Action endCallback = null, bool showDebug = false)
    {
        return caller.StartCoroutine(eventName, InvokeEventMultiThread_internal(caller, endCallback, showDebug));
    }
    private IEnumerator InvokeEventMultiThread_internal(MonoBehaviour caller, Action endCallback = null, bool showDebug = false)
    {
    ...
    private IEnumerator WrappedCall(MonoBehaviour caller, Func<IEnumerator> toCall, int callID)
    {
#if UNITY_EDITOR
        yield return caller.StartCoroutine(toCall.Method.Name, toCall());
#else
...
public class trackedEventSynchro<T1>
{
    ...
    public Coroutine InvokeEvent(MonoBehaviour caller, T1 arg, Action endCallback = null, bool showDebug = false)
    {
        return caller.StartCoroutine(eventName, InvokeEventMultiThread_internal(caller, arg, endCallback, showDebug));
    }
    private IEnumerator InvokeEventMultiThread_internal(MonoBehaviour caller, T1 arg, Action endCallback = null, bool showDebug = false)
    {
        if (_synchroAction != null && (trackedEventUtility.logInvokeEventCalls || showDebug))
        {
            Delegate[] dList = _synchroAction.GetInvocationList();
    ...
    private IEnumerator WrappedCall(MonoBehaviour caller, Func<T1, IEnumerator> toCall, T1 arg, int callID)
    {
#if UNITY_EDITOR
        yield return caller.StartCoroutine(toCall.Method.Name, toCall(arg));
#else
